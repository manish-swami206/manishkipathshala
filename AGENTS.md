...
- **Pagination contract**: Every paginated list (`.limit()/.offset()`) must ORDER BY a unique tiebreaker (usually the table's `id`) after the primary sort. Bulk inserts share one `now()` timestamp, and Postgres resolves ties differently per LIMIT/OFFSET query — without a tiebreaker, pages overlap and rows get skipped (admin sees fewer rows than `total`, and selected items reappear across pages).
- **Questions contract**: `POST /admin/questions/bulk-upload` returns `createdIds` (drives the CSV assign flow). `POST /admin/questions/assign` is append-only and atomic (transaction + FOR UPDATE + dedup, syncs `questionCount`/`totalQuestions`) — use it to ADD questions to a set/test; the PATCH endpoints replace `questionIds` wholesale.
...
- **Webhook contract**: `POST /api/webhooks/clerk` handles `user.created`, `user.updated`, `session.created`, `user.deleted`. Signature verified with Clerk's `verifyWebhook` from `@clerk/express/webhooks` (requires `CLERK_WEBHOOK_SIGNING_SECRET` env var — NOT `CLERK_WEBHOOK_SECRET`). The router is mounted in app.ts at `/api/webhooks/clerk`, so inside the router the route MUST be `router.post("/")` (Express strips the mount path — a nested `/webhooks/clerk` path 404s silently). All handlers respond immediately (200) and process DB writes async with `withWebhookRetry` (3 attempts, exponential backoff). `user.deleted` cleans up all 4 user-related tables in FK order.
...
- **Auth flow**: After signup redirect, `AppLayout` renders immediately once Clerk `isLoaded` — no global auth gate (an earlier `AuthLoadingGate` in `(app)/layout.tsx` was removed because it showed the stepper on every page). StreakTracker, NotificationPanel fire on mount (no idle gating). Support unread count is admin-only — not called in regular Header.
- **Signup stepper**: `SignupStepper.tsx` shows 3-step progress (Creating account → Setting up profile → Preparing dashboard). Shows in sign-up page while the Clerk SDK loads, and in BOTH sign-in and sign-up pages during the SSO callback phase via `useSsoCallbackStepper` (`hooks/useSsoCallback.tsx`) — Google OAuth finalizes on the auth pages with `?sso_callback=`/`?transfer=` params and shows a spinner for several seconds; the stepper replaces it. Auth pages wrap the hook in `Suspense` (required by `useSearchParams`).
