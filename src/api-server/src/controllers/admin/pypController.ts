...
export async function listAllPyp(req: Request, res: Response, next: NextFunction) {
  try {
    const { page = "1", limit = "20", search, subject, year: yearStr } = req.query as Record<string, string>;
    ...
    const data = await db
      ...
      .from(previousYearPapersTable)
      .where(where)
      .orderBy(desc(previousYearPapersTable.createdAt), desc(previousYearPapersTable.id))
      .limit(limitNum)
      .offset(offset);
